...
bool matchPassword(char *pass, unsigned char passwd[], unsigned char passmethod[]){
    ...
    case ENC_SHA256:
        ...
        sha256_update(&(context.sha256), (unsigned char*)pass, strlen(pass));
        sha256_update(&(context.sha256), passmethod, 3);
        sha256_finish(&(context.sha256),  newpasswd);
        break;
    default:
        return FALSE;
    }
    ...
    if( argc < 2 )
    {
        ...
        for( i = 0; i < 3; i++ )
        {
            ...
            else
            {
                ...
                for( j = 0; j < 1000; j++ )
                {
                    sha256_update( &ctx, (char *) buf, 1000 );
                }
            }
